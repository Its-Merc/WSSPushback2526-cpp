...
int8_t get_lerp_analog(pros::controller_analog_e_t axis, float t) {
    ...
    if (prev_axis == axis) return static_cast<int8_t>(std::lerp(prev, end, t));
...
int8_t get_modified_analog(pros::controller_analog_e_t axis, std::function<double(int8_t)> modifier) {
    return static_cast<int8_t>(modifier(ctrler.get_analog(axis)));
}
